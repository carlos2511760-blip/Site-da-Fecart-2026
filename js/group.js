(() => {
  ...
  function memberMarkup(member, index) {
    ...
    const visual = `<div class="demo-media-wrap">${member.photo ? image(member.photo, `Foto de ${name}`, 'member-photo', index < 2) : (poster ? image(poster, `Capa da prévia de ${name}`, 'member-photo', index < 2) : '<div class="member-photo member-photo-empty" aria-hidden="true"></div>')}${member.demo ? '<span class="demo-media-label">imagem de exemplo</span>' : ''}</div>`;
  ...
  function render(group, projects) {
    ...
    $('#group-content').innerHTML = `<section class="group-hero" style="--accent:${escapeHTML(group.accent || '#e8b63f')}"><div class="group-logo">${image(group.logo, `Logo do grupo ${group.name}`, 'logo-image')}</div><div><p class="section-kicker">${escapeHTML((group.copy||{}).pageKicker || 'grupo Fecart')}</p><h1>${escapeHTML(group.name)}</h1><p class="group-description">${escapeHTML(group.description || (group.copy||{}).missingDescription || 'Este grupo ainda está preparando sua apresentação.')}</p></div></section><section class="members-section section-block"><div class="block-heading"><p class="section-kicker">${escapeHTML((group.copy||{}).membersKicker || 'quem constrói')}</p><h2>${escapeHTML((group.copy||{}).membersTitle || 'Pessoas por\ntrás da ideia.').replace(/\n/g,'<br>')}</h2></div><div class="members-grid members-count-${memberCount}">${members.map(memberMarkup).join('')}</div></section><section class="group-project section-block"><div class="block-heading"><p class="section-kicker">${escapeHTML((group.copy||{}).projectKicker || 'o projeto')}</p><h2>${escapeHTML(project.title || (group.copy||{}).missingProject || 'Projeto em construção')}</h2></div><div class="project-detail"><div><div class="project-main-media">${project.image ? `<div class="demo-media-wrap">${image(project.image, `Foto do projeto ${project.title}`, 'project-main-image')}${project.demo ? '<span class="demo-media-label">imagem de exemplo</span>' : ''}</div>` : placeholderMedia((group.copy||{}).projectImagePlaceholder || 'Foto principal do projeto')}</div><p>${escapeHTML(project.description || (group.copy||{}).missingProjectDescription || 'A descrição detalhada do projeto será adicionada aqui.')}</p><p>${escapeHTML(project.details || '')}</p></div><div class="project-related">${projects.length ? projects.map(item => `<a href="index.html#projetos" class="related-project"><span>${escapeHTML(item.year || '')}</span><strong>${escapeHTML(item.title)}</strong><small>${escapeHTML((group.copy||{}).relatedLink || 'ver projeto completo')} ↗</small></a>`).join('') : `<p class="empty-detail">${escapeHTML((group.copy||{}).emptyRelated || 'Os projetos deste grupo aparecerão aqui.')}</p>`}</div></div></section><section class="gallery-section section-block"><div class="block-heading"><p class="section-kicker">${escapeHTML((group.copy||{}).galleryKicker || 'por trás de tudo')}</p><h2>${escapeHTML((group.copy||{}).galleryTitle || 'O processo\nem imagens.').replace(/\n/g,'<br>')}</h2><p>${escapeHTML((group.copy||{}).galleryDescription || 'Fotos e vídeos para registrar testes, protótipos, erros, descobertas e todas as etapas que fazem o projeto acontecer.')}</p></div><div class="process-gallery">${gallery.map(item => `<figure>${item.type === 'video' ? (item.src ? `<video class="gallery-video" src="${escapeHTML(item.src)}" controls preload="metadata"${item.poster ? ` poster="${escapeHTML(item.poster)}"` : ''}></video>` : placeholderMedia(item.caption || (group.copy||{}).galleryVideoPlaceholder || 'Vídeo do making of', 'video')) : item.src ? `<div class="demo-media-wrap">${image(item.src, item.alt || 'Registro do processo', 'gallery-image')}${item.demo ? '<span class="demo-media-label">imagem de exemplo</span>' : ''}</div>` : placeholderMedia(item.caption || (group.copy||{}).photoPlaceholder || 'Foto do processo')}<figcaption>${escapeHTML(item.caption || '')}</figcaption></figure>`).join('')}</div></section>`;
