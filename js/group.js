(() => {
  ...
  function memberMarkup(member, index) {
    ...
    const visual = `<div class="demo-media-wrap">${member.photo ? image(member.photo, `Foto de ${name}`, 'member-photo', index < 2) : (poster ? image(poster, `Capa da prévia de ${name}`, 'member-photo', index < 2) : '<div class="member-photo member-photo-empty" aria-hidden="true"></div>')}</div>`;
