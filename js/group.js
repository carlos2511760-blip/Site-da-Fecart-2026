(() => {
  ...
  function memberMarkup(member, index) {
    ...
    const visual = `<div class="demo-media-wrap">${member.photo ? image(member.photo, `Foto de ${name}`, 'member-photo', index < 2) : (poster ? image(poster, `Capa da prévia de ${name}`, 'member-photo', index < 2) : placeholderMedia(`Integrante ${index + 1}`, 'member'))}${member.demo ? '<span class="demo-media-label">imagem de exemplo</span>' : ''}</div>`;
