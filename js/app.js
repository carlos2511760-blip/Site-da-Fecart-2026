(() => {
  ...
  async function uploadAppFile(field) {
    ...
    const scope = field.dataset.uploadPath.startsWith('project:') ? `projects/${field.dataset.uploadPath.split(':')[1]}` : 'home'; const path = `${scope}/${field.dataset.uploadPath.replaceAll(':', '-')}-${Date.now()}.${extension}`;
    ...
    try {
      ...
      if (field.dataset.uploadPath.startsWith('project:')) { await saveProjectsToDatabase(); $('#editor-status').textContent = 'Mídia enviada e salva para todos.'; } else { await saveSiteToDatabase(); $('#editor-status').textContent = 'Arquivo enviado e salvo para todos.'; }
