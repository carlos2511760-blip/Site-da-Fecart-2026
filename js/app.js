(() => {
  ...
  function renderGroups() {
    $('#groups-grid').innerHTML = data.groups.map((group, index) => { const count = data.projects.filter(project => project.groupId === group.id).length; return `<article class="group-card" style="--accent:${escapeHTML(group.accent || '#e8b63f')}"><span class="group-index">0${index + 1} / 04</span><h3>${escapeHTML(group.name)}</h3>${group.description ? `<p>${escapeHTML(group.description)}</p>` : ''}<span class="group-count">${count} projeto${count === 1 ? '' : 's'}</span></article>`; }).join('');
  }
  function renderEditableCopy() { const values = { 'hero.title': data.site.hero?.title, 'hero.subtitle': data.site.hero?.subtitle, 'hero.manifesto': data.site.hero?.manifesto, 'about.description': data.site.about?.description }; Object.entries(values).forEach(([key, value]) => { const element = $(`[data-edit="${key}"]`); if (element) element.textContent = value || ''; }); }
  ...
  function renderEditor() {
    const hero = data.site.hero || {}, about = data.site.about || {}; let html = `<p class="section-kicker">textos principais</p>${editorField('Título do início', 'hero.title', hero.title)}${editorField('Subtítulo', 'hero.subtitle', hero.subtitle)}${editorField('Manifesto', 'hero.manifesto', hero.manifesto)}${editorField('Texto sobre a Fecart', 'about.description', about.description)}<p class="section-kicker">projetos</p><button class="button button-dark editor-add" id="editor-add-project" type="button">+ Adicionar projeto</button>`;
    data.projects.forEach(project => { html += `<div class="editor-group"><strong>${escapeHTML(project.title || 'Novo projeto')}</strong>${editorField('Título', `project:${project.id}:title`, project.title, 'input')}${editorField('Descrição curta', `project:${project.id}:shortDescription`, project.shortDescription)}${editorField('Descrição completa', `project:${project.id}:fullDescription`, project.fullDescription)}${editorField('Grupo', `project:${project.id}:groupId`, project.groupId, 'input')}${editorField('Ano', `project:${project.id}:year`, project.year, 'input')}${editorField('Status', `project:${project.id}:status`, project.status, 'input')}${editorField('Imagem (URL ou caminho)', `project:${project.id}:thumbnail`, project.thumbnail, 'input')}${editorField('Making Of em JSON', `project:${project.id}:roots`, JSON.stringify(project.roots || []), 'textarea')}</div>`; });
  ...
  function updateEditorPath(path, value) { if (path.startsWith('project:')) { const [, id, key] = path.split(':'); const project = data.projects.find(item => item.id === id); if (project) { if (key === 'roots') { try { project[key] = JSON.parse(value); } catch {} } else project[key] = value; } } else setByPath(path, value); persistDraft(); renderEditableCopy(); renderGroups(); renderProjects($('.filter-button.active')?.dataset.filter || 'all'); $('#editor-status').textContent = 'Alteração salva como rascunho local.'; }
