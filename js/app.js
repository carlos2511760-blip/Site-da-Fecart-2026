(() => {
  ...
  function renderEditor() {
    const hero = data.site.hero || {}, about = data.site.about || {}; let html = `<p class="section-kicker">textos principais</p>${editorField('Título do início', 'hero.title', hero.title)}${editorField('Subtítulo', 'hero.subtitle', hero.subtitle)}${editorField('Manifesto', 'hero.manifesto', hero.manifesto)}${editorField('Texto sobre a Fecart', 'about.description', about.description)}<p class="section-kicker">grupos</p>`;
    data.groups.forEach(group => { html += `<div class="editor-group"><strong>${escapeHTML(group.name)}</strong>${editorField('Descrição do grupo', `group:${group.id}:description`, group.description)}${editorField('Logo (URL ou caminho)', `group:${group.id}:logo`, group.logo, 'input')}${editorField('Integrantes em JSON', `group:${group.id}:members`, JSON.stringify(group.members || []), 'textarea')}${editorField('Projeto detalhado em JSON', `group:${group.id}:project`, JSON.stringify(group.project || {}), 'textarea')}${editorField('Galeria do processo em JSON', `group:${group.id}:gallery`, JSON.stringify(group.gallery || []), 'textarea')}</div>`; });
  ...
  function updateEditorPath(path, value) { if (path.startsWith('project:')) { const [, id, key] = path.split(':'); const project = data.projects.find(item => item.id === id); if (project) { if (key === 'roots') { try { project[key] = JSON.parse(value); } catch {} } else project[key] = value; } } else if (path.startsWith('group:')) { const [, id, key] = path.split(':'); const group = data.groups.find(item => item.id === id); if (group) { if (['members', 'project', 'gallery'].includes(key)) { try { group[key] = JSON.parse(value); } catch {} } else group[key] = value; } } else setByPath(path, value); persistDraft(); renderEditableCopy(); renderProjects($('.filter-button.active')?.dataset.filter || 'all'); renderGroups(); $('#editor-status').textContent = 'Alteração salva como rascunho local.'; }
