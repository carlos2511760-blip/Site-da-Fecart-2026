(() => {
  ...
  function updateEditorPath(path, value) { if (path.startsWith('project:')) { const [, id, key] = path.split(':'); const project = data.projects.find(item => item.id === id); if (project) { if (key === 'roots') { try { project[key] = JSON.parse(value); } catch {} } else project[key] = value; } } else setByPath(path, value); persistDraft(); renderEditableCopy(); renderProjects($('.filter-button.active')?.dataset.filter || 'all'); $('#editor-status').textContent = 'Alteração salva como rascunho local.'; }
  ...
  function renderAll() { renderEditableCopy(); renderFilters(); renderProjects(); }
