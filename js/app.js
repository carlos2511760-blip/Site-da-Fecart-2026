(() => {
  ...
  function renderEditor() {
    const hero = data.site.hero || {}, about = data.site.about || {}, theme = data.site.theme || {}; let html = `<p class="section-kicker">identidade visual</p>${editorField('Cor do papel', 'theme.paper', theme.paper || '#f4efe6', 'color')}${editorField('Cor do papel secundário', 'theme.paper-2', theme['paper-2'] || '#ebe2d3', 'color')}${editorField('Cor do texto', 'theme.ink', theme.ink || '#25231f', 'color')}${editorField('Cor de destaque', 'theme.tomato', theme.tomato || '#e05b3f', 'color')}${editorField('Cor solar', 'theme.sun', theme.sun || '#efc85d', 'color')}<p class="section-kicker">textos principais</p>${editorField('Título do início', 'hero.title', hero.title)}${editorField('Subtítulo', 'hero.subtitle', hero.subtitle)}${editorField('Texto sobre a Fecart', 'about.description', about.description)}<p class="section-kicker">grupos e equipes</p>`;
  ...
  function init() { $('#modal-close').addEventListener('click', closeProject); $('#project-modal').addEventListener('click', event => { if (event.target.id === 'project-modal') closeProject(); }); $('#editor-close').addEventListener('click', closeEditor); $('#editor-export').addEventListener('click', exportJSON); $('#editor-save-db').addEventListener('click', async () => { $('#editor-status').textContent = 'Salvando projetos, grupos e vídeos no banco público...'; try { await Promise.all([saveProjectsToDatabase(), saveGroupsToDatabase()]); $('#editor-status').textContent = 'Projetos, grupos e vídeos salvos para todos.'; } catch (error) { $('#editor-status').textContent = 'Não foi possível salvar no banco público.'; console.warn(error); } }); $('#editor-reset').addEventListener('click', resetEditor); $('#menu-toggle').addEventListener('click', () => { const open = $('#site-nav').classList.toggle('is-open'); $('#menu-toggle').setAttribute('aria-expanded', String(open)); }); $$('#site-nav a').forEach(link => link.addEventListener('click', () => $('#site-nav').classList.remove('is-open'))); document.addEventListener('keydown', event => { if (event.key === 'Escape') { closeProject(); closeEditor(); } const maintenanceKey = event.key === '\\' || event.key === '|' || event.code === 'Backslash' || event.code === 'IntlBackslash'; if (event.ctrlKey && event.shiftKey && maintenanceKey) { event.preventDefault(); $('#editor-panel').hidden ? openEditor() : closeEditor(); } }); }
