(() => {
  ...
  function renderFilters() {
    const copy = data.site.copy || {}; const groups = data.groups.map(group => `<button class="filter-button" type="button" data-filter="${escapeHTML(group.id)}">${escapeHTML(group.name)}</button>`).join('');
    $('#filter-bar').innerHTML = `<button class="filter-button active" type="button" data-filter="all">${escapeHTML(copy.filterAll || 'Todos')}</button>${groups}`;
  ...
  function renderGroups() {
    ...
    container.innerHTML = data.groups.map((group, index) => {
      ...
      const copy = data.site.copy || {};
      return `<article class="group-card" style="--accent:${escapeHTML(group.accent || '#e8b63f')}"><span class="group-index">${escapeHTML(copy.groupLabel || 'grupo')}</span><h3>${escapeHTML(group.name)}</h3><span class="group-count">${count} ${count === 1 ? escapeHTML(copy.projectSingular || 'projeto') : escapeHTML(copy.projectPlural || 'projetos')}</span><a class="group-card-link" href="grupo.html?grupo=${encodeURIComponent(group.id)}">${escapeHTML(copy.groupLink || 'Conhecer grupo')} <span>↗</span></a></article>`;
    }).join('');
  }
  ...
  function openProject(id) {
    ...
    $('#modal-content').innerHTML = `<div class="modal-content-grid"><div>${imageHTML(project.coverImage || project.thumbnail, project.title, 'modal-cover')}</div><div><span class="modal-label">${escapeHTML(group.name)} · ${escapeHTML(project.year || 'sem data')}</span><h2 class="modal-title" id="modal-title">${escapeHTML(project.title)}</h2><p class="modal-description">${escapeHTML(project.fullDescription || project.shortDescription)}</p><div class="tag-list">${(project.tags || []).map(tag => `<span class="tag">${escapeHTML(tag)}</span>`).join('')}</div><div class="modal-facts"><span><strong>${escapeHTML(copy.modalStatus || 'Status')}</strong><br>${escapeHTML(project.status || 'Em processo')}</span><span><strong>${escapeHTML(copy.modalTeam || 'Equipe')}</strong><br>${escapeHTML((project.team || []).join(', ') || 'A adicionar')}</span></div><a class="group-detail-link" href="grupo.html?grupo=${encodeURIComponent(project.groupId)}">${escapeHTML(copy.modalGroupLink || 'Conhecer o grupo')} <span>↗</span></a><button class="like-button ${liked ? 'is-liked' : ''}" id="like-button" type="button" ${liked ? 'disabled' : ''}>♡ <span>${liked ? escapeHTML(copy.modalLiked || 'Você curtiu') : escapeHTML(copy.modalLike || 'Curtir este projeto')}</span> · <b>${likeCount}</b></button></div></div><div class="timeline"><p class="modal-label">${escapeHTML(copy.modalMakingOf || 'Por trás de tudo · making of')}</p>${(project.roots || []).length ? (project.roots || []).map(root => `<div class="timeline-item"><time>${escapeHTML(root.date || '')}</time><div><strong>${escapeHTML(root.title || '')}</strong><p>${escapeHTML(root.description || '')}</p></div></div>`).join('') : `<p class="modal-description">${escapeHTML(copy.modalEmptyMakingOf || 'O making of deste projeto será registrado aqui.')}</p>`}</div>`;
  ...
  function renderEditor() {
    const hero = data.site.hero || {}, about = data.site.about || {}, theme = data.site.theme || {}, copy = data.site.copy || {}; let html = `<div class="editor-section"><p class="section-kicker">identidade visual</p>${editorField('Cor do papel', 'theme.paper', theme.paper || '#f4efe6', 'color')}${editorField('Cor do papel secundário', 'theme.paper-2', theme['paper-2'] || '#ebe2d3', 'color')}${editorField('Cor do texto', 'theme.ink', theme.ink || '#25231f', 'color')}${editorField('Cor de destaque', 'theme.tomato', theme.tomato || '#e05b3f', 'color')}${editorField('Cor solar', 'theme.sun', theme.sun || '#efc85d', 'color')}</div><div class="editor-section"><p class="section-kicker">página inicial</p>${editorField('Título principal', 'hero.title', hero.title)}${editorField('Subtítulo principal', 'hero.subtitle', hero.subtitle)}${editorField('Botão principal', 'copy.heroAction', copy.heroAction, 'input')}${editorField('Texto sobre a Fecart', 'about.description', about.description)}${editorField('Título da seção Sobre', 'copy.aboutTitle', copy.aboutTitle)}${editorField('Rótulo da seção Sobre', 'copy.aboutKicker', copy.aboutKicker, 'input')}</div><div class="editor-section"><p class="section-kicker">navegação, projetos e quiz</p>${editorField('Link Início', 'copy.navHome', copy.navHome, 'input')}${editorField('Link Projetos', 'copy.navProjects', copy.navProjects, 'input')}${editorField('Link Sobre', 'copy.navAbout', copy.navAbout, 'input')}${editorField('Rótulo de projetos', 'copy.projectsKicker', copy.projectsKicker, 'input')}${editorField('Título de projetos', 'copy.projectsTitle', copy.projectsTitle)}${editorField('Subtítulo de projetos', 'copy.projectsNote', copy.projectsNote)}${editorField('Texto quando não há projetos', 'copy.emptyProjects', copy.emptyProjects)}${editorField('Rótulo do quiz', 'copy.quizKicker', copy.quizKicker, 'input')}${editorField('Título do quiz', 'copy.quizTitle', copy.quizTitle)}${editorField('Descrição do quiz', 'copy.quizDescription', copy.quizDescription)}${editorField('Botão do quiz', 'copy.quizButton', copy.quizButton, 'input')}</div><div class="editor-section"><p class="section-kicker">rodapé e modais</p>${editorField('Instagram', 'copy.footerInstagram', copy.footerInstagram, 'input')}${editorField('TikTok', 'copy.footerTiktok', copy.footerTiktok, 'input')}${editorField('Copyright', 'copy.footerCopyright', copy.footerCopyright, 'input')}${editorField('Título do Making Of', 'copy.modalMakingOf', copy.modalMakingOf, 'input')}${editorField('Making Of vazio', 'copy.modalEmptyMakingOf', copy.modalEmptyMakingOf)}${editorField('Botão Conhecer o grupo', 'copy.modalGroupLink', copy.modalGroupLink, 'input')}${editorField('Botão Curtir', 'copy.modalLike', copy.modalLike, 'input')}</div><div class="editor-section"><p class="section-kicker">grupos e equipes</p>`;
    data.groups.forEach(group => { const memberCount = group.id === 'sabor-robotica' ? 4 : 5; html += `<div class="editor-group"><strong>${escapeHTML(group.name)}</strong>${editorField('Nome do grupo', `group:${group.id}:name`, group.name, 'input')}${editorField('Cor do grupo', `group:${group.id}:accent`, group.accent || '#e8b63f', 'color')}${editorField('Logo (URL ou caminho)', `group:${group.id}:logo`, group.logo, 'input')}${editorUpload('Ou envie o logo do computador', `group:${group.id}:logo`, 'image/jpeg,image/png,image/webp, image/svg+xml')}${editorField('Descrição do grupo', `group:${group.id}:description`, group.description)}<p class="section-kicker">integrantes</p>`; for (let index = 0; index < memberCount; index += 1) { const member = group.members?.[index] || {}; html += `<div class="editor-group"><strong>Integrante ${index + 1}</strong>${editorField('Nome', `group:${group.id}:member:${index}:name`, member.name, 'input')}${editorField('Função', `group:${group.id}:member:${index}:role`, member.role, 'input')}${editorField('Foto (URL ou caminho)', `group:${group.id}:member:${index}:photo`, member.photo, 'input')}${editorUpload('Ou envie a foto do computador', `group:${group.id}:member:${index}:photo`, 'image/jpeg,image/png,image/webp')}${editorField('Vídeo de prévia (URL do YouTube, MP4 ou WEBM)', `group:${group.id}:member:${index}:previewVideo`, member.previewVideo, 'input')}${editorUpload('Ou envie a prévia do computador', `group:${group.id}:member:${index}:previewVideo`, 'video/mp4,video/webm,video/quicktime')}${editorField('Capa do vídeo (URL ou caminho)', `group:${group.id}:member:${index}:videoPoster`, member.videoPoster, 'input')}${editorUpload('Ou envie a capa do computador', `group:${group.id}:member:${index}:videoPoster`, 'image/jpeg,image/png,image/webp')}</div>`; } const project = group.project || {}; html += `<p class="section-kicker">projeto e Making Of</p>${editorField('Nome do projeto', `group:${group.id}:project:title`, project.title, 'input')}${editorField('Foto principal', `group:${group.id}:project:image`, project.image, 'input')}${editorField('Descrição curta', `group:${group.id}:project:description`, project.description)}${editorField('Descrição detalhada', `group:${group.id}:project:details`, project.details)}${editorField('Galeria (JSON avançado)', `group:${group.id}:gallery`, JSON.stringify(group.gallery || [], null, 2))}</div>`; });
    html += `<div class="editor-section"><p class="section-kicker">rótulos auxiliares</p>${editorField('Rótulo dos grupos', 'copy.groupLabel', copy.groupLabel, 'input')}${editorField('Botão dos grupos', 'copy.groupLink', copy.groupLink, 'input')}${editorField('Projeto no singular', 'copy.projectSingular', copy.projectSingular, 'input')}${editorField('Projeto no plural', 'copy.projectPlural', copy.projectPlural, 'input')}${editorField('Acessibilidade do card', 'copy.openProject', copy.openProject, 'input')}${editorField('Texto sem data', 'copy.noDate', copy.noDate, 'input')}${editorField('Curtida no singular', 'copy.likeSingular', copy.likeSingular, 'input')}${editorField('Final das curtidas', 'copy.likePlural', copy.likePlural, 'input')}</div><div class="editor-section"><p class="section-kicker">projetos</p><button class="button button-dark editor-add" id="editor-add-project" type="button">+ Adicionar projeto</button>`;
    data.projects.forEach(project => { html += `<div class="editor-group"><strong>${escapeHTML(project.title || 'Novo projeto')}</strong><button class="button button-danger editor-delete" type="button" data-delete-project="${escapeHTML(project.id)}">Excluir este projeto</button>${editorField('Título', `project:${project.id}:title`, project.title, 'input')}${editorField('Descrição curta', `project:${project.id}:shortDescription`, project.shortDescription)}${editorField('Descrição completa', `project:${project.id}:fullDescription`, project.fullDescription)}${editorGroupField(`project:${project.id}:groupId`, project.groupId)}${editorField('Ano', `project:${project.id}:year`, project.year, 'input')}${editorField('Status', `project:${project.id}:status`, project.status, 'input')}${editorField('Imagem principal (URL ou caminho)', `project:${project.id}:thumbnail`, project.thumbnail, 'input')}${editorField('Imagem de capa (URL ou caminho)', `project:${project.id}:coverImage`, project.coverImage, 'input')}${editorField('Tags em JSON', `project:${project.id}:tags`, JSON.stringify(project.tags || []), 'textarea')}${editorField('Equipe em JSON', `project:${project.id}:team`, JSON.stringify(project.team || []), 'textarea')}${editorField('Making Of em JSON', `project:${project.id}:roots`, JSON.stringify(project.roots || []), 'textarea')}</div>`; });
    html += '</div>';
    $('#editor-fields').innerHTML = html; $('#editor-save-db').disabled = !supabase.url; $('#editor-add-project').addEventListener('click', addProject); $$('.editor-delete').forEach(button => button.addEventListener('click', () => deleteProject(button.dataset.deleteProject))); $$('[data-path]').forEach(field => field.addEventListener('input', () => updateEditorPath(field.dataset.path, field.value))); $$('[data-upload-path]').forEach(field => field.addEventListener('change', () => uploadAppFile(field)));
  }
  async function uploadAppFile(field) {
    const file = field.files?.[0];
    if (!file || !supabase.url || !supabase.anonKey) return;
    if (file.size > 80 * 1024 * 1024) { $('#editor-status').textContent = 'O arquivo deve ter no máximo 80 MB.'; field.value = ''; return; }
    const extension = (file.name.split('.').pop() || 'bin').toLowerCase().replace(/[^a-z0-9]/g, '');
    const path = `editor/${field.dataset.uploadPath.replaceAll(':', '-')}-${Date.now()}.${extension}`;
    field.disabled = true; $('#editor-status').textContent = 'Enviando arquivo para o armazenamento público...';
    try {
      ...
      updateEditorPath(field.dataset.uploadPath, `${supabase.url}/storage/v1/object/public/fecart-media/${path}`);
      if (field.dataset.uploadPath.includes(':member:')) { await saveGroupsToDatabase(); $('#editor-status').textContent = 'Foto enviada e salva para todos.'; } else if (field.dataset.uploadPath.includes(':logo')) { await saveGroupsToDatabase(); $('#editor-status').textContent = 'Logo enviado e salvo para todos.'; } else $('#editor-status').textContent = 'Arquivo enviado. Clique em salvar para publicar para todos.';
    } catch (error) { $('#editor-status').textContent = 'Não foi possível enviar o arquivo.'; console.warn(error); }
  ...
  async function deleteProject(id) { const project = data.projects.find(item => item.id === id); if (!project) return; if (!window.confirm(`Excluir definitivamente o projeto “${project.title || 'Sem título'}” para todos?`)) return; const button = $(`[data-delete-project="${CSS.escape(id)}"]`); if (!supabase.url || !supabase.anonKey) { $('#editor-status').textContent = 'Supabase não configurado: o projeto não foi excluído para o público.'; return; } button.disabled = true; const response = await fetch(apiUrl(`fecart_projects?slug=eq.${encodeURIComponent(project.id)}`), { method: 'DELETE', headers: { ...apiHeaders(), Prefer: 'return=minimal' } }); if (!response.ok) { button.disabled = false; $('#editor-status').textContent = 'Não foi possível excluir para todos. Execute o SQL atualizado do Supabase e tente novamente.'; return; } data.projects = data.projects.filter(item => item.id !== id); localStorage.removeItem(`fecart-liked-project-${id}`); persistDraft(); renderAll(); renderEditor(); $('#editor-status').textContent = 'Projeto excluído publicamente para todos.'; }
  async function saveSiteToDatabase() { if (!supabase.url || !supabase.anonKey) throw new Error('Supabase não configurado'); const response = await fetch(apiUrl('fecart_site_content?on_conflict=site_id'), { method: 'POST', headers: { ...apiHeaders(), Prefer: 'resolution=merge-duplicates,return=minimal' }, body: JSON.stringify({ site_id: 'home', content: data.site }) }); if (!response.ok) throw new Error(`Falha ao salvar textos do site (${response.status})`); }
  async function saveGroupsToDatabase() {
    if (!supabase.url || !supabase.anonKey) throw new Error('Supabase não configurado');
    const rows = data.groups.map(group => ({ group_id: group.id, content: group }));
    const response = await fetch(apiUrl('fecart_group_content?on_conflict=group_id'), { method: 'POST', headers: { ...apiHeaders(), Prefer: 'resolution=merge-duplicates,return=minimal' }, body: JSON.stringify(rows) });
    if (!response.ok) throw new Error(`Falha ao salvar grupos (${response.status})`);
  }
  ...
  function init() { $('#modal-close').addEventListener('click', closeProject); $('#project-modal').addEventListener('click', event => { if (event.target.id === 'project-modal') closeProject(); }); $('#editor-close').addEventListener('click', closeEditor); $('#editor-export').addEventListener('click', exportJSON); $('#editor-save-db').addEventListener('click', async () => { $('#editor-status').textContent = 'Salvando projetos, grupos e vídeos no banco público...'; try { await Promise.all([saveProjectsToDatabase(), saveGroupsToDatabase(), saveSiteToDatabase()]); $('#editor-status').textContent = 'Projetos, grupos e vídeos salvos para todos.'; } catch (error) { $('#editor-status').textContent = 'Não foi possível salvar no banco público.'; console.warn(error); } }); $('#editor-reset').addEventListener('click', resetEditor); $('#menu-toggle').addEventListener('click', () => { const open = $('#site-nav').classList.toggle('is-open'); $('#menu-toggle').setAttribute('aria-expanded', String(open)); }); $$('#site-nav a').forEach(link => link.addEventListener('click', () => $('#site-nav').classList.remove('is-open'))); document.addEventListener('keydown', event => { if (event.key === 'Escape') { closeProject(); closeEditor(); } const maintenanceKey = event.key === '\\' || event.key === '|' || event.code === 'Backslash' || event.code === 'IntlBackslash'; if (event.ctrlKey && event.shiftKey && maintenanceKey) { event.preventDefault(); $('#editor-panel').hidden ? openEditor() : closeEditor(); } }); }
